...
void  EKATFT::begin(uint16_t id, int8_t shift, uint8_t mirr_h, uint8_t mirr_v, uint8_t rotate) {

    uint32_t startaddr;
    uint16_t linewidth;

#ifdef EKA_TFT_VER_1   // 3.5' 320x240 TFT
    _page=0;
...
// Rotate 0
    if (rotate == 0) {
      ...
      _get_max_x      = DISP_HOR_RESOLUTION - 1;
      _get_max_y      = DISP_VER_RESOLUTION - 1;
      startaddr = 0;
    }
// Rotate 90
    if (rotate == 1) {
      _line_mem_pitch = DISP_VER_RESOLUTION;
      _get_max_x      = DISP_VER_RESOLUTION - 1;
      _get_max_y      = DISP_HOR_RESOLUTION - 1;
      startaddr = (((uint32_t) _get_max_x + 1) >> 1) - 1;
    }
// Rotate 180
    if (rotate == 2) {
      _line_mem_pitch = DISP_HOR_RESOLUTION;
      _get_max_x      = DISP_HOR_RESOLUTION - 1;
      _get_max_y      = DISP_VER_RESOLUTION - 1;
      startaddr = ((((uint32_t) _get_max_x + 1) * (_get_max_y + 1)) >> 1) - 1;
    }
// Rotate 270
    if (rotate == 3) {
      _line_mem_pitch = DISP_VER_RESOLUTION;
      _get_max_x      = DISP_VER_RESOLUTION - 1;
      _get_max_y      = DISP_HOR_RESOLUTION - 1;
      startaddr = (((uint32_t) _get_max_x + 1) * _get_max_y) >> 1;
    }
    ...
    _get_max_x      = DISP_HOR_RESOLUTION - 1;
    _get_max_y      = DISP_VER_RESOLUTION - 1;
    reset(); delay(2); wrReg8(0xA0,0x00); delay(100);
    MainWndEnable(false);
    MainWndInit(0,320,0x04,0,1);
    FloatWndInit(253600L, 1, 1, 1, 1, 1, 1);
    if (rotate == 0) _line_mem_pitch=DISP_HOR_RESOLUTION;
    else             _line_mem_pitch=DISP_HOR_RESOLUTION; // DISP_VER_RESOLUTION
    MainWndEnable(1);
    wrReg8(0xA8,0x1F); wrReg8(0xA9,0x80);
    gpio_bl(0);delay(10);gpio_bl(1);gpio_rst(0);delay(10);gpio_rst(1);delay(10);gpio_rst(1);
    wrReg8(0x126,0x0A); wrReg8(0x127,0xB4); wrReg8(0x12B,0xAE); wrReg8(0x126,0x8A); wrReg8(0x04,0x00);
    delay(20); wrReg8(0x158,0xFF); wrReg8(0x159,0xFF); wrReg8(0x15A,0x0F); wrReg8(0x10,0x55);
    wrReg8(0x250,0x20); wrReg8(0x260,0xC8); wrReg8(0x250,0x80); wrReg8(0x250,0x20); wrReg8(0x252,0x01);
    wrReg8(0x270,0x00); wrReg8(0x271,0x00); wrReg8(0x12,312>>3); wrReg8(0x13,0x00); wrReg8(0x14,312>>3);
    wrReg8(0x18,239-1); wrReg8(0x19,240%8); wrReg8(0x1C,239); wrReg8(0x1D,240%8); wrReg8(0x261,0x02);
    wrReg8(0x74,0x00); wrReg8(0x75,0x00); wrReg8(0x76,0x00); wrReg8(0x71,0x40); wrReg8(0x78,160);
    wrReg8(0x79,0x0); wrReg8(0x70,0x04); wrReg8(0x1A4,0xc0);
    LSC(0x01); delay(20); LSA(0xC0,0x23); LSA(0xC1,0x10); LSE(0xC5,0x3E,0x28); LSA(0xC7,0x86);
    LSC(0x36); uint8_t dr=0x38; if(mirr_h) dr|=0x80; if(mirr_v) dr|=0x40; LSD(dr); LSA(0x3A,0x55);
    LSE(0xB1,0x00,0x18); LSE(0xB6,0x08,0x82); LSB(0x27,0x00); LSE(0x2A,0x00,0x00); LSB(0x01,0x3F);
    LSE(0x2B,0x00,0x00); LSB(0x00,0xEF); LSA(0x26,0x01); LSE(0xE0,0x0F,0x31); LSB(0x2B,0x0C);
    LSB(0x0E,0x08); LSB(0x4E,0xF1); LSB(0x37,0x07); LSB(0x10,0x03); LSB(0x0E,0x09); LSD(0x00);
    LSE(0xE1,0x00,0x0E); LSB(0x14,0x03); LSB(0x11,0x07); LSB(0x31,0xC1); LSB(0x48,0x08);
    LSB(0x0F,0x0C); LSB(0x31,0x36); LSD(0x0F); LSE(0x37,0x00,0x00); LSC(0x11); delay(10); LSC(0x29);
    LCD_SetCursorPosition(0,0,319,239); wrReg8(0xa0,0x00); LSC(0x2C);
    int32_t n=310L*2L+640L*239L+(int32_t)shift; for(int32_t i=0; i<n; i++) LSD(0x0F);
    LCD_SetWriteModeSPI(AUTO_REFRESH_MODE);
#else
    reset(); delay(2); wrReg8(0xA0,0x00); delay(100);
    MainWndEnable(false); MainWndInit(0,320,0x04,0,1);
    FloatWndInit(253600L, 1, 1, 1, 1, 1, 1); FocusWnd(0); MainWndEnable(1);
    wrReg8(0xA8,0x1F); wrReg8(0xA9,0x80);
    gpio_bl(0);delay(10);gpio_bl(1);gpio_rst(0);delay(10);gpio_rst(1);delay(10);gpio_rst(1);
    wrReg8(0x126,0x0A); wrReg8(0x127,0xB4); wrReg8(0x12B,0xAE); wrReg8(0x126,0x8A); wrReg8(0x04,0x00);
    delay(20); wrReg8(0x158,0xFF); wrReg8(0x159,0xFF); wrReg8(0x15A,0x0F); wrReg8(0x10,0x55);
...
// Rotate 0
    if (rotate == 0) {
      ...
      startaddr = 0;
//       wrReg8(0x71, (0x40|0));
//      wrReg32(0x74, (uint32_t)0);
//      wrReg16(0x78,(uint16_t)DISP_HOR_RESOLUTION>>1);
    }
// Rotate 90
    if (rotate == 1) {
      _line_mem_pitch = DISP_VER_RESOLUTION;
      ...
      startaddr = (((uint32_t) _get_max_x + 1) >> 1) - 1;
//      wrReg8(0x71, (0x40|1));
//      wrReg32(0x74, (uint32_t)((((uint32_t) _get_max_x + 1) >> 1) - 1));
//      wrReg16(0x78,(uint16_t)DISP_VER_RESOLUTION>>1);
    }
// Rotate 180
    if (rotate == 2) {
      _line_mem_pitch = DISP_HOR_RESOLUTION;
      ...
      startaddr = ((((uint32_t) _get_max_x + 1) * (_get_max_y + 1)) >> 1) - 1;
//      wrReg8(0x71, (0x40|2));
//      wrReg32(0x74, (uint32_t)((((uint32_t) _get_max_x + 1) * (_get_max_y + 1)) >> 1) - 1);
//      wrReg16(0x78,(uint16_t)DISP_HOR_RESOLUTION>>1);
    }
// Rotate 270
    if (rotate == 3) {
      ...
      startaddr = (((uint32_t) _get_max_x + 1) * _get_max_y) >> 1;
//      wrReg8(0x71, (0x40|3));
//      wrReg32(0x74, (uint32_t)((((uint32_t) _get_max_x + 1) * _get_max_y) >> 1));
//      wrReg16(0x78,(uint16_t)DISP_VER_RESOLUTION>>1);
    }
//    MainWndInit(0,320,0x04,0,1);
    MainWndInit(startaddr, _line_mem_pitch, 0x04, rotate, 1);
...
#else
wrReg8(0x26e,0x01);
    int32_t n=310L*2L+640L*239L+(int32_t)shift; for(int32_t i=0; i<n; i++) {
        if(i&2)
        {
            if(i&1) wrReg16(0x26c,0x34); // Light
            else    wrReg16(0x26c,0xA5);
        } else {
            if(i&1) wrReg16(0x26c,0xEB); // Dark
            else    wrReg16(0x26c,0x5A);
        }
    };
#endif
    LCD_SetWriteModeSPI(AUTO_REFRESH_MODE);
...
#ifdef EKA_TFT_VER_4   // 2.8' 320x240 TFT
    uint8_t  i = 0; gpioStatus = 0; wrReg8(0x250, 0x80);
    reset(); delay(10); wrReg8(0xA0, 0x00); delay(200);
    wrReg8(0xA8, 0x1F); wrReg8(0xA9, 0x80);
    gpio_bl(0); gpio_spena(1); gpio_spclk(1); gpio_spdat(1); gpio_sprst(1);
    ...
    gpio_spclk(0); gpio_bl(1); wrReg8(0x126, 0x0A); wrReg8(0x127, 0xB4);
    wrReg8(0x12B, 0xAE); wrReg8(0x126, 0x8A); wrReg8(0x04, 0x00); delay(20);
    wrReg32(0x158, 0x13FFF); wrReg8(0x10, 0x61); wrReg8(0x11, 0x00);
    wrReg8(0x12, 0x17); wrReg8(0x13, 0x01); wrReg8(0x22, 0x01);	wrReg8(0x23, 0x00);
    wrReg8(0x20, 0x02); wrReg8(0x21, 0x01);	wrReg8(0x16, 0x04); wrReg8(0x17, 0x00);
    wrReg8(0x14, 0x1D); wrReg8(0x18, 0xB2);	wrReg8(0x19, 0x01); wrReg8(0x1C, 0x3F);
    wrReg8(0x1D, 0x01);	wrReg8(0x1E, 0x1E);	wrReg8(0x1F, 0x00); wrReg8(0x24, 2);
    wrReg8(0x26, 30); wrReg8(0x27, 0x00); wrReg8(0xA0, 0x00);
    MainWndEnable(false);
    FloatWndInit(253600L, 1, 1, 1, 1, 1, 1);
    _page=0;
// Rotate 0
...
// Rotate 270
    ...
    MainWndInit(startaddr, _line_mem_pitch, 0x04, rotate, 1);
    MainWndEnable(true);
#endif // EKA_TFT_VER_4   // 2.8' 320x240 TFT
}

#ifdef EKA_TFT_VER_2   // 320x240 TFT Arduino Nano
void  EKATFT::LCD_SetCursorPosition(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    LCD_SendCommand(0x2A); LCD_SendData(x1>>8); LCD_SendData(x1); LCD_SendData(x2>>8); LCD_SendData(x2);
    LCD_SendCommand(0x2B); LCD_SendData(y1>>8); LCD_SendData(y1); LCD_SendData(y2>>8); LCD_SendData(y2);
}
#endif
...
uint16_t  EKATFT::DrawArc(int16_t xL, int16_t yT, int16_t xR, int16_t yB, int16_t r1, int16_t r2, uint8_t octant) {
...
uint16_t  EKATFT::DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {
    ...
    wrReg8(0x1F8, (_get_max_x + 1) & 0xFF); wrReg8(0x1F9, ((_get_max_x + 1) >> 8) & 0xFF);
    wrReg8(0x1D8, (_get_max_y + 1) & 0xFF); wrReg8(0x1D9, ((_get_max_y + 1) >> 8) & 0xFF);
...
void  EKATFT::DrawBevel(int16_t x1, int16_t y1, int16_t x2, int16_t y2, int16_t rad) {
...
uint16_t  EKATFT::DrawFillBevel(int16_t x1, int16_t y1, int16_t x2, int16_t y2, int16_t rad) {
...
void  EKATFT::DrawPoly(int16_t numPoints, int16_t* polyPoints) {
...
void  EKATFT::DrawFillRect(int16_t left, int16_t top, int16_t right, int16_t bottom) {
    uint16_t sw;
    ...
    wrReg8(0x1F5, 0); wrReg8(0x1F6, 0); wrReg8(0x1F8, (_get_max_x + 1) & 0xFF);
    wrReg8(0x1F9, ((_get_max_x + 1) >> 8) & 0xFF); wrReg8(0x1D8, (_get_max_y + 1) & 0xFF);
    wrReg8(0x1D9, ((_get_max_y + 1) >> 8) & 0xFF); wrReg8(0x1FE, RED8(_color));
...
void  EKATFT::PutPixel(int16_t x, int16_t y) {
...
void  EKATFT::PutPixels(uint32_t address, uint16_t * buff, uint16_t length) {
    uint8_t  value;  uint16_t  i; SetAddress(address); RD_CD_IDLE; WR_ACTIVE;
    for (i = 0; i < length; i++) { WriteData(*buff); buff++;} WR_IDLE; CS_IDLE;
}
...
uint8_t  INTRFC::rdReg8(uint16_t r) {
    ...
    RD_ACTIVE; setReadDir(); WR_CD_IDLE; CS_ACTIVE; _NOP(); _NOP(); CS_IDLE; read8(x); RD_IDLE; setWriteDir();
    return x;
} 

uint16_t  INTRFC::rdReg16(uint16_t r) {
    static WORD_VAL x; RD_IDLE; WR_CD_ACTIVE; write8(0x00); write8((r) >> 8); write8(r);
    RD_ACTIVE; setReadDir(); WR_CD_IDLE; CS_ACTIVE; _NOP(); _NOP(); CS_IDLE; read8(x.v[0]); read8(x.v[1]);
...
void  EKATFT::MainWndInit(uint32_t startaddr, uint16_t linewidth, uint16_t bpp, uint8_t orient, uint8_t rgb) {
    wrReg32(0x74, (uint32_t)startaddr);
    wrReg8(0x70, (rdReg8(0x70)|bpp)); wrReg16(0x78,(uint16_t)linewidth>>1);
    wrReg8(0x71, ((rdReg8(0x71)|0x40|orient))); wrReg8(0x1A4, ((rdReg8(0x1A4)&0xC0)|0x40));
    if(rgb) wrReg8(0x1A4, (rdReg8(0x1A4)&0xC0)|0x40);
    else    wrReg8(0x1A4, rdReg8(0x1A4) & ~0x40);
}

void  EKATFT::FloatWndInit(uint32_t startaddr, uint16_t linewidth, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t rgb) {
#if (ROTATION == 0)
...
void  EKATFT::FocusWnd(uint8_t wnd) {
    uint16_t  linewidth;
...
void  EKATFT::SetFont(const GFXfont *f) {
...
#if defined(ESP32)
uint16_t  EKATFT::OutText(const char* textString) {
#else
uint16_t  EKATFT::OutText(unsigned char* textString) {
#endif
...
void  EKATFT::OutChar(unsigned char  ch) {
//    GLYPH_ENTRY * pChTable;  uint8_t * pChImage, temp, mask, adv, w;
//    int8_t  yo, yA; int16_t  chWidth, xCnt, yCnt, x, y;
    GLYPH_ENTRY * pChTable;  uint8_t * pChImage, temp, mask, adv;
    int8_t  yo, yA; int16_t  chWidth, xCnt, yCnt, x, y, w = 0;
...
#if defined(ESP32)
int16_t  EKATFT::GetTextWidth(const char * textString, const void* font) {
#else
int16_t  EKATFT::GetTextWidth(char * textString, void* font) {
#endif
...
int16_t  EKATFT::GetTextHeight(void* font) {
...
#if defined(ESP32)
uint16_t  EKATFT::OutTextXY(int16_t x, int16_t y, const char * textString) {
#else
uint16_t  EKATFT::OutTextXY(int16_t x, int16_t y, char * textString) {
#endif
