...
#define SCREEN_HOR_SIZE                           320 //320
#define SCREEN_VER_SIZE                           240 //240
#define LINE_MEM_PITCH                            320 //320 
#else
#define DISP_HOR_RESOLUTION                       320
#define DISP_VER_RESOLUTION                       240
...
//#define  setReadDirInline()   {pinMode(LCD_D0,INPUT);pinMode(LCD_D1,INPUT);pinMode(LCD_D2,INPUT);pinMode(LCD_D3,INPUT);pinMode(LCD_D4,INPUT);pinMode(LCD_D5,INPUT);pinMode(LCD_D6,INPUT);pinMode(LCD_D7,INPUT);}
//#define  setWriteDirInline()  {pinMode(LCD_D0,OUTPUT);pinMode(LCD_D1,OUTPUT);pinMode(LCD_D2,OUTPUT);pinMode(LCD_D3,OUTPUT);pinMode(LCD_D4,OUTPUT);pinMode(LCD_D5,OUTPUT);pinMode(LCD_D6,OUTPUT);pinMode(LCD_D7,OUTPUT);}
...
//#define BL_IDLE    digitalWrite( LCD_BL,  LOW)
//#define BL_ACTIVE  digitalWrite( LCD_BL, HIGH)
...
*	7	6	5	4	3	2	1	0
*
*	13	12	14	27	16	17	25	26
*
*
*	27	26	25	17	16	14	13	12
*
*	23	26	24	15	13	9	6	6
*
*********************************************************************************/
void  write8 (uint8_t value)
{
    ...
    uint32_t  reg_hi = 0, reg_lo;

    if (value & 0x01)  reg_hi = reg_hi | LCD_D0_PIN;
    else               reg_lo = reg_lo | LCD_D0_PIN;
    if (value & 0x02)  reg_hi = reg_hi | LCD_D1_PIN;
    else               reg_lo = reg_lo | LCD_D1_PIN;
    if (value & 0x04)  reg_hi = reg_hi | LCD_D2_PIN;
    else               reg_lo = reg_lo | LCD_D2_PIN;
    if (value & 0x08)  reg_hi = reg_hi | LCD_D3_PIN;
    else               reg_lo = reg_lo | LCD_D3_PIN;
    if (value & 0x10)  reg_hi = reg_hi | LCD_D4_PIN;
    else               reg_lo = reg_lo | LCD_D4_PIN;
    if (value & 0x20)  reg_hi = reg_hi | LCD_D5_PIN;
    else               reg_lo = reg_lo | LCD_D5_PIN;
    if (value & 0x40)  reg_hi = reg_hi | LCD_D6_PIN;
    else               reg_lo = reg_lo | LCD_D6_PIN;
    if (value & 0x80)  reg_hi = reg_hi | LCD_D7_PIN;
...
//                           _NOP(); _NOP(); _NOP(); _NOP(); _NOP(); _NOP(); CS_ACTIVE; _NOP(); _NOP(); _NOP(); _NOP(); _NOP(); _NOP(); CS_IDLE;}


//#define readport8inline(result) { result = ((PINB & B00000111) | (PIND & B11111000)); }

//#define readport8inline(result)   { uint32_t  shift=READ_PERI_REG(GPIO_IN_REG);
//#define readport8inline(result)   { uint32_t  shift=LCD_D0_PIN|LCD_D2_PIN|LCD_D4_PIN|LCD_D6_PIN;

#if defined(EKA_TFT_VER_2) || defined(EKA_TFT_VER_3)  /* Break Board for Arduino Nano, ESP-32 */ 
#define readport8inline(result)   { uint32_t  shift=READ_PERI_REG(GPIO_IN_REG);            \
                                    shift=READ_PERI_REG(GPIO_IN_REG);                      \
                                    uint32_t  result8 = 0;                                 \
                                    ...
                                    (result) = result8;}
#else // EKA_TFT_VER_1, EKA_TFT_VER_4
#define readport8inline(result)   { uint32_t  shift=READ_PERI_REG(GPIO_IN_REG);            \
                                    uint32_t  result8 = 0;                                 \
                                    if (shift & LCD_D0_PIN) result8 = (result8 | 0x01);    \
                                    if (shift & LCD_D1_PIN) result8 = (result8 | 0x02);    \
...
#if 0
// Set value of TFT register: 16-bit address, 8-bit value
#define writeRegister8inline(a, d) {                      \
    uint8_t hi, lo;                                       \
    hi = (a) >> 8; lo = (a); CD_COMMAND; WR_ACTIVE; RD_IDLE; \
    write8(0); write8(hi); write8(lo);                    \
    CD_DATA; write8(d); /*CD_DATA;*/ WR_IDLE; /*RD_IDLE;*/ \
}

// Set value of TFT register: 16-bit address, 16-bit value
// See notes at top about macro expansion, hence hi & lo temp vars
#define writeRegister16inline(a, d) {                     \
    /*uint8_t hi, lo; hi = (d) >> 8; lo = (d);*/          \
    writeRegister8(a,/*lo*/(d));                          \
    writeRegister8(a+1,/*hi*/(d)>>8);                     \
}

// LCD BACKLIGHT
#define gpio_blInline(a) {                                \
    if(a) gpioStatus = gpioStatus | LCD_BLIGHT;           \
    else  gpioStatus = gpioStatus & (~LCD_BLIGHT);        \
    writeRegister8(REG_GPIO_STATUS_CONTROL0, gpioStatus); \
}

// LCD SPI RESET
#define gpio_sprstInline(a) {                             \
    if(a) gpioStatus = gpioStatus | LCD_RESET;            \
    else  gpioStatus = gpioStatus & (~LCD_RESET);         \
    writeRegister8(REG_GPIO_STATUS_CONTROL0, gpioStatus); \
}

// LCD SPI ENA
#define gpio_spenaInline(a) {                             \
    if(a) gpioStatus = gpioStatus | LCD_SPENA;            \
    else  gpioStatus = gpioStatus & (~LCD_SPENA);         \
    writeRegister8(REG_GPIO_STATUS_CONTROL0, gpioStatus); \
}

// LCD SPI CLK
#define gpio_spclkInline(a) {                             \
    if(a) gpioStatus = gpioStatus | LCD_SPCLK;            \
    else  gpioStatus = gpioStatus & (~LCD_SPCLK);         \
    writeRegister8(REG_GPIO_STATUS_CONTROL0, gpioStatus); \
}

// LCD SPI DAT
#define gpio_spdatInline(a) {                             \
    if(a) gpioStatus = gpioStatus | LCD_SPDAT;            \
    else  gpioStatus = gpioStatus & (~LCD_SPDAT);         \
    writeRegister8(REG_GPIO_STATUS_CONTROL0, gpioStatus); \
}
...
#if 1 /* !!! */
// LCD SPI Write
#define spi_writeInline(byte) {                \
    uint8_t  bit_ctr;                          \
    for(bit_ctr = 0; bit_ctr < 8; bit_ctr++) { \
        if(byte & 0x80) gpio_spdat(1);         \
        else            gpio_spdat(0);         \
        gpio_spclk(0);                         \
        gpio_spclk(1);                         \
        byte=(byte<<1);                        \
...
// LCD SPI Set Reg Word
#define spi_setregwInline(reg, cmd) {          \
    uint8_t hi, lo;                            \
    hi = (cmd) >> 8; lo = (cmd);               \
    gpio_spena(0);                             \
    spi_write(0x70);                           \
...
// LCD SPI Set Reg Byte
#define spi_setregbInline(reg, cmd) {          \
    uint16_t _val;                             \
    uint8_t hi, lo;                            \
    _val = reg;                                \
    _val = ((_val<<10)|0x0200);                \
...
//#ifdef EKA_TFT_VER_3   // 320x240 TFT ESP32
//    void        LCD_SendCommand(uint8_t cmd) {wrReg8(0x26e,0x00);wrReg16(0x26c,cmd);};
//    void        LCD_SendData(uint8_t data) {wrReg8(0x26e,0x01);wrReg16(0x26c,data);};
//    void        LCD_SetWriteModeSPI(uint8_t mode) {if(mode==0) wrReg8(0x260,0xC8);else if(mode==1) wrReg8(0x260,0x48);};
//    void        LCD_SetCursorPosition(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2);
...
// Add !!!
    //void        PutMemBuff(uint32_t address, uint8_t * buff, uint16_t length);
    void        PutPixels(uint32_t address, uint16_t * buff, uint16_t length);
...
#endif
//#ifdef EKA_TFT_VER_2 || EKA_TFT_VER_3   // 320x240 TFT Arduino Nano
#ifndef gpio_rst
