...
#endif
    uint16_t entry; uint16_t chk; uint16_t attributes; uint32_t dirclus; uint32_t dirccls;
}
#if defined (ESP32)
__attribute__ ((__packed__))
#endif
...
typedef struct { uint8_t PTE_BootDes;SWORD PTE_FrstPartSect;uint8_t PTE_FSDesc;SWORD PTE_LstPartSect;uint32_t PTE_FrstSect;uint32_t PTE_NumSect;}
#if defined (ESP32)
__attribute__ ((__packed__))
#endif
 PTE_MBR;

typedef struct { uint8_t ConsChkRtn[446];PTE_MBR Partition0;PTE_MBR Partition1;PTE_MBR Partition2;PTE_MBR Partition3;uint8_t Signature0;uint8_t Signature1;}
#if defined (ESP32)
__attribute__ ((__packed__))
#endif
 _PT_MBR;
...
typedef struct { union{ _BPB_FAT32  FAT_32; _BPB_FAT16  FAT_16; _BPB_FAT12  FAT_12;} FAT;
    uint8_t  Reserved[512-sizeof(_BPB_FAT32)-2]; uint8_t  Signature0; uint8_t  Signature1;}
#if defined (ESP32)
__attribute__ ((__packed__))
#endif
 _BootSec;
...
typedef struct {uint8_t errorCode; union{uint8_t value;struct{uint8_t sectorSize:1;uint8_t maxLUN:1;}bits;}validityFlags;uint16_t sectorSize;uint8_t maxLUN;}
#if defined (ESP32)
__attribute__ ((__packed__))
#endif
 MEDIA_INFORMATION;
...
class  SDC:INTRFC {
  public:
    ...
    uint8_t     JPEGWriteToSD(uint32_t fsize, char * file_name);
