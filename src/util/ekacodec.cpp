...
void  CODEC::JPEGSetRegs(uint32_t jpeg_width, uint32_t jpeg_height, uint32_t raw_jpeg_addr, uint32_t shift) {
    uint32_t vpixel, hpixel; uint8_t vpixel_8, hpixel_8;
    uint32_t exp_jpeg_width, exp_jpeg_height; uint32_t line_buf, vert_buf;
    uint32_t encoded_jpeg_addr=(uint32_t)raw_jpeg_addr + (uint32_t)shift;
