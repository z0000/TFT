...
FSFILE * SDC::FSfopen( const char * fileName, const char *mode ) {
    ...
    if (SDWriteProtectState() && (ModeC != 'r') && (ModeC != 'R')) {
// !!!
//Serial.println("CE_WRITE_PROTECTED");
        FSerrno = CE_WRITE_PROTECTED;
        return NULL;
    }
    filePtr = NULL;
    for (fIndex = 0; fIndex < FS_MAX_FILES_OPEN; fIndex++) {
        if (gFileSlotOpen[fIndex]) { gFileSlotOpen[fIndex] = FALSE;
        filePtr = &gFileArray[fIndex];
// !!!
//Serial.println("gFileArray");
        break;}
    }
    if (filePtr == NULL) {
        FSerrno = CE_TOO_MANY_FILES_OPEN;
// !!!
//Serial.println("CE_TOO_MANY_FILES_OPEN");
        return NULL;
    }
#if defined(SUPPORT_LFN)
...
// !!!
//Serial.println("CE_INVALID_FILENAME");
        return NULL;
    }
...
size_t  SDC::FSfwrite(const void *data_to_write, size_t size, size_t n, FSFILE *stream) {
    ...
    while ((error == CE_GOOD) && (count > 0)) {
        ...
        if (pos == dsk->sectorSize) {
            ...
            if (stream->sec == dsk->SecPerClus) {
                stream->sec = 0;
                if (stream->flags.FileWriteEOF) { error = (CETYPE)FileAllocateNewCluster(stream, 0); needRead = FALSE;}
                else  error = (CETYPE)FileGetNextCluster( stream, 1);
            }
...
size_t  SDC::FSfread (void *ptr, size_t size, size_t n, FSFILE *stream) {
    ...
    while( len )
    {
        if (seek == stream->size) { FSerrno = CE_EOF; error = (CETYPE)CE_EOF; break;}
        // In fopen, pos is init to 0 and the sect is loaded
        if (pos == MEDIA_SECTOR_SIZE) {
            pos = 0; stream->sec++;
            if (stream->sec == dsk->SecPerClus) {
                stream->sec = 0;
                if ((error = (CETYPE)FileGetNextCluster( stream, 1)) != CE_GOOD) {
...
#endif // SUPPORT_LFN
        if ((error = (CETYPE)PopulateEntries(fo, fHandle, mode)) == CE_GOOD) {
...
#endif
        {}
        if (error == CE_GOOD) { error = (CETYPE)EraseCluster(disk,*cluster);}
    }
    return(error);
}
#endif // ALLOW_WRITES
...
#if defined (ESP32)
uint8_t reg; int8_t (*pf)()=(int8_t(*)())f; uint8_t af=0; reg=rdReg8(0x71); wrReg8(0x71,reg&0xBF);
#else
...
#else
#define  FILE_BUFF_SIZE   128
#endif

uint8_t  SDC::JPEGWriteToSD(uint32_t fsize, char * file_name) {
    FSFILE *pFile=NULL;
    uint8_t  jpeg_encode_byte;
    uint32_t i=0, len;
    uint32_t en_jpeg_address=rdReg32(0x414)<<2;
    pFile = FSfopen(file_name, FS_WRITE);
    if (pFile != NULL) {
        uint8_t file_buff[FILE_BUFF_SIZE];
        if (fsize >= FILE_BUFF_SIZE) {
            len = FILE_BUFF_SIZE;
            for (i=0; i<fsize; i+=len) {
                if ((fsize-i)>FILE_BUFF_SIZE) len=FILE_BUFF_SIZE;
                else len=fsize-i;
                GetMemBuff(en_jpeg_address+i,&file_buff[0],len);
                if (FSfwrite(&file_buff[0], len, 1, pFile)!=1) { return 0;}
                else {}
            }
        } else {
            GetMemBuff(en_jpeg_address, &file_buff[0], fsize);
            if (FSfwrite(&file_buff[0], fsize, 1, pFile)!=1) return 0;
        }
        FSfclose(pFile);
        return 1;
    } else { return 0;}
...
JPEG_ERR  SDC::SDJPEGHeader(JPEG_DECODE * decode) {
    ...
    while (cnt++ < 250)
    {
        ...
        if (decode->bytes_read < decode->size) {
            if (!SDFile2JPEGFIFO(JPEG_FIFO_START_ADDR, JPEG_FIFO_SIZE, (FSFILE*)decode->stream)) { }
